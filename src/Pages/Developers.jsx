...
function Skills({ skills }) {
  ...
  return (
    <div className="w-25">
      ...
      {skills.length > 5 && (
        <Button variant="info" className="text-light" onClick={toggleShowMore}>
          ...
          (profiles?.length > 0 ? (
            <ul className="list-group">
              {profiles.map((profile) => (
                <li
                  className="list-group-item d-flex gap-3 align-items-center my-3 border bg-secondary-subtle p-4"
                  key={profile._id}
                >
                  <img
                    width={225}
